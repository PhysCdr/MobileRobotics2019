...
def sample_motion_model(odometry, particles):
    ...
    return [{'x': x[i], 'y': y[i], 'theta': theta[i]} for i in range(len(particles))]
...
def eval_sensor_model(sensor_data, particles, landmarks):
    ...
    for particle in particles:
        ...
        for landmark_id, distance in zip(ids, ranges):
            landmark_x = landmarks[landmark_id][0]
            landmark_y = landmarks[landmark_id][1]
            likelihood *= np.exp(-(np.sqrt((x - landmark_x)**2 +
                                 (y - landmark_y)**2) - distance)**2 / 2 / sigma_r**2)
