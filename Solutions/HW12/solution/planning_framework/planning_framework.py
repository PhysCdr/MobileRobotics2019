...
def get_neighborhood(cell, occ_map_shape):
  ...
  for x in [cell[0]-1, cell[0], cell[0]+1]:
  	for y in [cell[1]-1, cell[1], cell[1]+1]:
  		if ((x == cell[0]) and (y == cell[0])) or x<0 or y<0 or x>=occ_map_shape[0] or y>=occ_map_shape[1]:
  			continue
  		else:
...
def get_edge_cost(parent, child, occ_map):
  ...
  '''your code here'''
  threshold = 0.5
  d = np.sqrt(np.sum((parent - child)**2 ))
...
def get_heuristic(cell, goal):
  ...
  '''your code here'''
  # a straight line is always an admissible heuristic since h(n)<=h*(n)
  return np.linalg.norm(cell - goal)
  '''***        ***'''
...
def run_path_planning(occ_map, start, goal):
  ...
  while not np.array_equal(parent, goal):
    ...
    '''your code here'''
    neighbors = get_neighborhood([x,y], occ_map.shape)
    for neighbor in neighbors:
    	cost_neighbor = costs[parent[0], parent[1]] + get_edge_cost(parent, neighbor, occ_map)
    	if cost_neighbor < costs[neighbor]:
    		costs[neighbor] = cost_neighbor
